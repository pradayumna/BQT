---
title: "Business Quantitative Techniques Project"
...
list.of.packages <- c("qwraps2", "formattable", "knitr", "foreign", "ggplot2", 'sandwich', 'lmtest', 'dplyr', 'tinytex') #We need these packages to run the code
...
```{r year range validation,echo=FALSE}
...
\newpage
Lindo et al Table 1 from original study^1^:  
```{r}
if(file.exists("Table 1.png")) knitr::include_graphics(rep("Table 1.png")) 
...
The scale location plot suggest the residuals of the data do not have constant variance as the width of the data spread appears to increase as X increases.  
...
data1A %>% 
  ...
  geom_smooth(method = "glm", formula = y ~ poly(x, 2), se = FALSE, data = data1A[data1A$dist_from_cut_med10 > 0, ])+
  geom_smooth(method = "glm", formula = y ~ poly(x, 2), se = FALSE, data = data1A[data1A$dist_from_cut_med10 < 0, ])+
  geom_vline(xintercept = 0) + 
  labs(y = "Frequency count", x = "GPA Distance from Cutoff", ggtitle("Figure 2: Distribution of Student GPAs"))
```
\newpage
Figure 2 replicates Figure 1 in the original report^1^.  
```{r}
#knitr::include_graphics(rep("C:/Users/Louise/OneDrive/Documents/ECOM5002/R data/Frequency plot.png"))
```
...
Table3 = do.call(rbind.data.frame, answer)
colnames(Table3) <- c('GPA below cutoff (1)', 'P-Value (1)', 'Std.err (1)','Intercept (0)', 'P-Value (0)', 'Std.err (0)')
rownames(Table3) <- myvars
kable(Table3, caption = 'Observable Characteristics Estimated Discontinuity')
```
\newpage
Table 3 above replicates Lindo et al Table 2 in the original report^1^.  
```{r}
#knitr::include_graphics(rep("C:/Users/Louise/OneDrive/Documents/ECOM5002/R data/Table 2.png"))
```
...
```{r}
#knitr::include_graphics(rep("C:/Users/Louise/OneDrive/Documents/ECOM5002/R data/equation.png"))
```
...
{
  model3 = lm(formula = nextGPA ~ gpalscutoff*dist_from_cut, data = x)
...
kable(Table4, caption = 'Next GPA Assessment')
```
...
\newpage
Lindo et al Table 6 and 7 ^1^  
```{r}
#knitr::include_graphics(rep("C:/Users/Louise/OneDrive/Documents/ECOM5002/R data/Table 6.png"))
```

```{r}
#knitr::include_graphics(rep("C:/Users/Louise/OneDrive/Documents/ECOM5002/R data/Table 7.png"))
```
...
plot(glm3)
```
This improves the consistency of the residuals variance however does not improve the fit to a normal distribution. Further transformations of the output variable 'nextGPA' are required to improve the fit of the data.  
...
\newpage
### References
1. Replication of Lindo, Jason M., Nicholas J. Sanders, and Philip Oreopoulos. 2010. "Ability, Gender, and Performance Standards: Evidence from Academic Probation."   
American Economic Journal: Applied Economics, 2 (2): 95-117.DOI: 10.1257/app.2.2.95  
...
Essen, Germany
Last updated on Wednesday, October 06, 2021

3. Replication of Lindo, Sanders & Oreopoulos (2010), Student Project.  
https://github.com/amageh/replication-performance-standards/blob/master/replication-notebook.ipynb  
last updated 19 Apr 2020  

4. A Practical Introduction to Regression Discontinuity Designs: Volume II
Matias D. Cattaneo, Nicol´as Idrobo, Roc´ıo Titiunik
June 14, 2018
